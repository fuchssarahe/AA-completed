...
class Array
  def bubble_sort!(&blk)
    bubble_sorted = false
    blk ||= Proc.new { |num1, num2| num1 <=> num2 }
...
def substrings(string)
  ...
  (0...string.length).each do |idx|
    range_end = idx
    while range_end < string.length
      res << string[idx..range_end]
      range_end += 1
    end
  end
...
def subwords(word, dictionary)
  potential_words = substrings(word)
  potential_words.select { |word| dictionary.include?(word)}
end
